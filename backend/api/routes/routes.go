package routes

import (
	"net/http"

	database "proyecto2/backend/db"
)

func RegisterRoutes(mux *http.ServeMux, manager *database.Manager) {
	mux.HandleFunc("GET /api/health", Health(manager))
	mux.HandleFunc("POST /api/login", LoginEmployee(manager))
	mux.HandleFunc("POST /api/productos", CreateProduct(manager))
	mux.HandleFunc("GET /api/productos", IndexProducts(manager))
	mux.HandleFunc("GET /api/productos/{id}", ShowProduct(manager))
	mux.HandleFunc("PUT /api/productos/{id}", UpdateProduct(manager))
	mux.HandleFunc("PATCH /api/productos/{id}", PatchProduct(manager))
	mux.HandleFunc("DELETE /api/productos/{id}", DestroyProduct(manager))
	mux.HandleFunc("POST /api/categorias", CreateCategory(manager))
	mux.HandleFunc("GET /api/categorias", ListCategories(manager))
	mux.HandleFunc("GET /api/categorias/{id}", ShowCategory(manager))
	mux.HandleFunc("PUT /api/categorias/{id}", UpdateCategory(manager))
	mux.HandleFunc("DELETE /api/categorias/{id}", DestroyCategory(manager))
	mux.HandleFunc("POST /api/proveedores", CreateProvider(manager))
	mux.HandleFunc("GET /api/proveedores", ListProviders(manager))
	mux.HandleFunc("GET /api/proveedores/{id}", ShowProvider(manager))
	mux.HandleFunc("PUT /api/proveedores/{id}", UpdateProvider(manager))
	mux.HandleFunc("DELETE /api/proveedores/{id}", DestroyProvider(manager))
	mux.HandleFunc("POST /api/empleados", CreateEmployee(manager))
	mux.HandleFunc("GET /api/empleados", ListEmployees(manager))
	mux.HandleFunc("GET /api/empleados/{id}", ShowEmployee(manager))
	mux.HandleFunc("PUT /api/empleados/{id}", UpdateEmployee(manager))
	mux.HandleFunc("DELETE /api/empleados/{id}", DestroyEmployee(manager))
	mux.HandleFunc("POST /api/clientes", CreateClient(manager))
	mux.HandleFunc("GET /api/clientes", ListClients(manager))
	mux.HandleFunc("GET /api/clientes/{id}", ShowClient(manager))
	mux.HandleFunc("PUT /api/clientes/{id}", UpdateClient(manager))
	mux.HandleFunc("DELETE /api/clientes/{id}", DestroyClient(manager))
	mux.HandleFunc("POST /api/compras", CreateCompra(manager))
	mux.HandleFunc("GET /api/compras", IndexCompras(manager))
	mux.HandleFunc("GET /api/compras/{id}", ShowCompra(manager))
	mux.HandleFunc("PUT /api/compras/{id}", UpdateCompra(manager))
	mux.HandleFunc("DELETE /api/compras/{id}", DestroyCompra(manager))
}

func Health(manager *database.Manager) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		status := http.StatusOK
		databaseStatus := "ok"

		if err := manager.Ping(r.Context()); err != nil {
			status = http.StatusServiceUnavailable
			databaseStatus = "error"
		}

		WriteJSON(w, status, map[string]string{
			"status":   "ok",
			"backend":  "ok",
			"database": databaseStatus,
		})
	}
}
